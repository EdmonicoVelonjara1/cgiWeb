...
#include "message_auth.h"
 ...
 int main(){
   ...
   // printf("totalPages= %d\n",totalPages);
    ...
    printf("</table></center>\n");
     if (currentPage > 1) {
        printf("<a href=\"/cgi-bin/m.cgi?page=%d&items=%d\"><button>Page précédente</button></a> ",currentPage - 1, itemsPerPage);
    }
    for(int i=0;i<totalPages;i++){
        ...
        if((i+1)%10 == (endIndex-startIndex)){
            startIndex = endIndex +1;
            //currentPage = startIndex + (i+1)%10;
        }
        if(i<endIndex && i>=startIndex){
            if(startIndex+i==endIndex)startIndex = endIndex;
            printf(
                        "<a href=\"/cgi-bin/m.cgi?page=%d&items=%d\">"
                        "<button>%d</button></a>", currentPage,item, currentPage
                );
           /*  int t=0;
